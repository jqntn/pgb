...
impl Rect {
    ...
    fn iter_tiles(self) -> impl Iterator<Item = (u16, u16)> {
        ...
        y_range_incl
            .map(move |y_index| x_range_incl.clone().map(move |x_index| (x_index, y_index)))
            .flatten()
    }
}
...
extern "C" fn main() -> ! {
    ...
    OBJ_ATTR0
        .iter()
        .skip(creatures.len())
        .for_each(|va| va.write(no_display));
    ...
    loop {
        ...
        for (i, (creature_pos, attr_addr)) in creatures.iter().zip(OBJ_ATTR_ALL.iter()).enumerate()
        {
